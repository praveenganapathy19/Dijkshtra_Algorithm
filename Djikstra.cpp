...
//Comparator designed to convert Max-Heap priority queue to Min-Heap priority queue
class Compare
{
    public:
	    bool operator() (pair<Vertex*, float> a, pair<Vertex*, float> b) {
	    	return b.second < a.second;
	    }
};

//Priority queue object creation
//priority_queue<pair<Vertex*, float>, vector < pair<Vertex*, float> >, Compare> pq;
...
void Graph::print()
{
    ...
    for(itr = vertexMap.begin(); itr!=vertexMap.end(); itr++)
    {
        ...
        for(it = vertex->adj.begin(); it!=vertex->adj.end(); it++)
        {
            ...
            OutputFile<<"\t"<<adjacentVertex->name<<" "<<edge->length;
            if(!(edge->up))
            {
                cout<<" DOWN";
                OutputFile<<" DOWN";
...
int main( int argc, char *argv[ ] )
{
    ...
    ifstream inFile( "social.txt" );
    if( !inFile )
    {
        cerr << "Cannot open 1 " << argv[ 1 ] << endl;
        return 1;
    }
    ...
    ifstream QueryFile( "queries.txt" );
    
    if( !QueryFile )
    {
        cerr << "Cannot open 2" << argv[ 3 ] << endl;
        return 1;
    }
    cout<< "\nReading from query file\n";
    while(processRequest( QueryFile, g) )
        ;
    inFile.close();
    QueryFile.close();
    OutputFile.close();
